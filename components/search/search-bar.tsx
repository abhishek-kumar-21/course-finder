...
export function SearchBar({ topic, setTopic, onSearch, loading, canSearch }: SearchBarProps) {
  return (
    <form onSubmit={onSearch} className="relative group z-20">
      ...
      <div className="relative flex items-center bg-[#0a0a0a]/80 backdrop-blur-xl border border-slate-800 rounded-xl p-2 shadow-2xl focus-within:border-emerald-500/50 transition-colors">
        ...
        <input
          ...
          className="w-full bg-transparent border-none text-white placeholder-slate-500 focus:ring-0 px-4 py-4 text-lg outline-none font-medium"
          value={topic}
          onChange={(e) => setTopic(e.target.value)}
        />
        <button
          type="submit"
          // Disable if loading, topic is empty, OR no language is selected
          disabled={loading || !topic.trim() || !canSearch}
